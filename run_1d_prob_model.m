...
phi_dif = zeros(length(x),length(t));
psi_dif = zeros(length(x),length(t));
...
for ii = 1:length(t)-1
    ...
    % stimulus magnitude
    s_p = -K*phi_p.^2./(1-phi_p).^4 + M*Da*phi_p.*(1 - c_p);
    % source term -- momentum equation
    F_p = (exp(-2*s_p).*s_p + exp(-2*s_p) + s_p - 1)./(s_p.*(1 - exp(-2*s_p)));
    F_p(s_p < 1e-4) = 1/3*s_p(s_p < 1e-4);  % limit - prevents singularity
    % source term -- phi_diff equation
    S_dif_p = (1 - exp(-s_p))./(1 + exp(-s_p));
    % source term -- psi_diff equation
    F_dif_p = (exp(-s_p).*s_p + exp(-s_p) + s_p - 1)./(s_p.*(exp(-s_p) + 1));
    F_dif_p(s_p < 1e-4) = 1/2 + 1/24*(s_p(s_p < 1e-4)).^2;   % limit - prevents singularity
...
figure(4)
plot(x,phi_dif(:,ind_t)/phi(:,ind_t))
xlabel('x')
ylabel('\phi^{dif}')
